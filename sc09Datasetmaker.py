...
pathToBaseDatasetFolder = 'Data/dataset/'
folderNames = ['sc09/train'] #['test']# , 'test', 'valid']
...
for directory in dirs:
    ...
    for file_name in os.listdir(directory):
        if True:#counter<100: #load 250 files max
            ...
            filelistnames = ''
            audios = np.array([])
            files = random.sample(allfiles,37)
            for file in files:
                audio, sr = librosa.load(directory + '/' + file, sr=None, dtype=np.float64)
                #print(file)
                if (len(audios)>0):
            ...
            audio=audios
            file = hashlib.md5(filelistnames.encode()).hexdigest()
            #print(len(audio))
...
#
            ...
            if np.sum(np.absolute(audio)) < len(audio) * 1e-4:
                print('newFile', "doesn't meet the minimum amplitude requirement")
                continue
            ...
            with h5py.File(dir + str(num) + '/large.hdf5', 'a') as hf:
                try:
                    hf.create_dataset(str(file), data=toSave, compression=None)
                    hf.close()
                except:
...
#with h5py.File(dir + str(num) + '/large.h5', 'w') as hf:
